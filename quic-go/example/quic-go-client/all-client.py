...
MAX_LEN = 1000000
HEADER_LEN = 12
server_ip = "172.31.3.254"
local_ip = "172.31.13.36"
server_port = 4242
local_port = 4243
MTU=1480
...
def udp_request(filename, target_path):
    ...
    while True:
        try:
            ...
            if current_frame_no < frame_no:  # 高序号
                break
            if current_frame_no > frame_no:  # 低序号
                continue
            if bucket[packet_no] == 1:  # 重复发送的包
                continue
            #if packet_no % 1000 == 0:
            ...
            print("Timeout Detected. Request Next Frame!")
            ifTimeout = 1
            break
    if ifTimeout:
        print("Timeout. Received packets:", num_packets)
    else:
...
def stream_with_manifest(manifest_path):
    ...
    with open('data.txt', 'w') as file:
        for chunk in manifest:
            ...
            file.write("Receive Frame on:" + str(datetime.datetime.now()) + "\n")
...
if __name__=="__main__":
    stream_with_manifest("manifest_all.json")
