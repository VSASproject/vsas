...
//server one session
func serveOne(stream quic.Stream, dirPath string, file_name string, indentifier bool) error {
	//Send的格式
	if indentifier{
		...
		content_length_str := fillString(strconv.FormatInt(-400,10),10)
		fmt.Println("Wrong Length is %s", content_length_str)
		_, err := stream.Write([]byte(content_length_str))
		if err != nil {
			panic(err)
		}
...
func dashServer() error {
	...
	//for{//
		...
		write.WriteString("]")
		write.Flush()
		serveOne(stream, ".", filePath, true)
    	quic_send(stream, string(message))
	//}
	return err
}

func quic_send(stream quic.Stream, dirPath string) {
